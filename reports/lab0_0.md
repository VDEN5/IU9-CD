...
function parse(program) {
  ...
  function parseBody() {
    ...
    while (peek() !== null && peek() !== 'end' && peek() !== 'endif'
&& peek() !== 'do' && peek() !== 'wend') {
      const token = peek();
...
/**
 * Форматирует результат парсинга в строку, похожую на Scheme-выражение
...
(parse "1 x dup while dup 0 > do 1 - swap over * swap wend")
...
⇒ ((((-- 1 -)
   (=0? dup 0 =)
   (=1? dup 1 =)
   (factorial =0? (if (drop 1 exit)) =1? (if (drop 1 exit)) 1 swap (while (dup 0 >)
(1 - swap over * swap)) drop))
...
# Вывод
Вспомнил метод рекурсивного спуска с основ информатики 1 семестра, по факту идея в заглядывании
на некоторое число символов вперед и однозначном определении применяемого правила
(при том грамматика должна позволять сделать это, то есть быть LL(k)),
затем мы съедаем токены и идем далее рекурсивно разбирать части.